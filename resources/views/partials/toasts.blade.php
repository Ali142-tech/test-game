...
@once('toast-scripts')
    ...
    (() => {
        ...
        const bindToasts = () => {
            document.querySelectorAll('[data-toast]:not([data-toast-bound])').forEach((toast) => {
                toast.setAttribute('data-toast-bound', '1');
                toast._autoTimer = setTimeout(() => dismiss(toast), 5200);
            });
        };
        ...
        document.addEventListener('click', (event) => {
            ...
            dismiss(closeBtn.closest('[data-toast]'));
        });

        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', bindToasts);
        } else {
            bindToasts();
        }
    })();
